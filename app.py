...
st.sidebar.info(
    ...
    2. Select a record if your file contains multiple records.
...
def extract_features_from_genbank(genbank_content, feature_types=['CDS', 'tRNA', 'gene']):
    """Extracts specified features from GenBank content and returns all records."""
    text_stream = StringIO(genbank_content.decode("utf-8")) if isinstance(genbank_content, bytes) else StringIO(genbank_content.getvalue().decode("utf-8"))
...
if uploaded_file is not None:
    try:
        records = extract_features_from_genbank(uploaded_file)
        
        if records:
            # Display record selection if there are multiple records
            if len(records) > 1:
                ...
                selected_record_index = st.selectbox(
                    ...
                    help="Select which record to analyze from the GenBank file."
                )
                record = records[selected_record_index]
                st.success(f"Selected record: {record.id}")
            else:
                record = records[0]
                st.success(f"Loaded record: {record.id}")
                
            # Extract features from the selected record
            features = extract_features_from_record(record)
            
            st.write("## Feature Selection")
            feature_types_available = [ft for ft in ['CDS', 'tRNA', 'gene'] if features.get(ft) and len(features.get(ft)) > 0]
            
            if not feature_types_available:
                st.warning("No supported features (CDS, tRNA, gene) found in this record.")
            else:
                feature_type = st.selectbox(
                    'Select feature type:', 
                    feature_types_available,
                    help="Choose the type of genomic feature for which you want to design primers."
                )

                if features[feature_type]:
                    # Create a more informative display for feature selection
                    feature_options = []
                    for feature in features[feature_type]:
                        gene_name = feature.qualifiers.get('gene', ['Unknown'])[0]
                        product_name = feature.qualifiers.get('product', [''])[0]
                        display_text = f"{gene_name} - {product_name} ({feature.location})"
                        feature_options.append(display_text)
                    ...
                    # Add a toggle to show/hide sequence
                    if st.checkbox("Show sequence", value=False):
        ...
        st.error(f"Error processing the GenBank file: {str(e)}")
        st.info("Please ensure your file is a valid GenBank format file.")
