...
def simulate_pcr(forward_primer, reverse_primer, sequence, max_product_size=3000):
    ...
    forward_matches = [m.start() for m in re.finditer(forward_primer, sequence)]
    # Find all reverse primer binding sites (complement)
    reverse_primer_rc = str(Seq(reverse_primer).reverse_complement())
    reverse_matches = [m.start() for m in re.finditer(reverse_primer_rc, sequence)]
...
def check_primer_specificity(forward_primer, reverse_primer, records, target_record_id, target_feature=None, max_product_size=3000):
    ...
    for record in records:
        record_id = record.id
        is_target = (record_id == target_record_id)
        
        # Skip large-scale analysis on the target record
        if is_target and target_feature:
            # Only check the target feature region if specified
            ...
            # For other records, check against features of interest to avoid whole sequence
            ...
            if features[feature_type]:
                ...
                if st.button(f'Design Primers for selected {feature_type}'):
                    ...
                    if primer_data:
                        ...
                        # Primer specificity check option (only if multiple records are present)
                        if len(records) > 1 and 'primer_data' in st.session_state:
                            ...
                            if st.button("Check Primer Specificity"):
                                ...
                                if specificity_results:
                                    ...
                                    # Count non-target amplifications
                                    ...
                                    st.success("No potential amplification products found in other sequences.")
                    else:
